...
* @include qmp_use.c
*/
void QMPool_put(QMPool * const me, void *b, char enISR) {
    
    /** @pre # free blocks cannot exceed the total # blocks and
    * the block pointer must be from this pool.
    */
	uint32_t basepri = seccion_critica_entrar(enISR); //Enter on critical section

    ((QFreeBlock *)b)->next = (QFreeBlock *)me->free_head;/* link into list */
    me->free_head = b;      /* set as new head of the free list */
    ++me->nFree;            /* one more free block in this pool */
    
    seccion_critica_salir(enISR,basepri); //Exit from critical section
}
...
* @include qmp_use.c
*/
void *QMPool_get(QMPool * const me, uint_fast16_t const margin, char enISR) {
    QFreeBlock *fb;
...
uint32_t seccion_critica_entrar ( char enISR )
{
    uint32_t basepri = 0;
    if( enISR ) {
...
void seccion_critica_salir ( char enISR, uint32_t basepri )
{
    if( enISR ) {
